...
class Day17 extends Solution:
  ...
  override def part1(ctx: Context): Int =
    findMinimumLoss(
      ctx,
      selectDirections = (current, straights) =>
        Directions.filter(d => d != current * -1 && (d != current || straights < 3))
    )
    ...
    findMinimumLoss(
      ...
      selectDirections = (current, straights) =>
        straights match
          ...
          case c if c < 10         => Directions.filter(d => d != current * -1)
          case c if c == 10        => Directions.filter(d => d != current * -1 && d != current)
    )

  def findMinimumLoss(
      ctx: Context,
      endCondition: (straights: Int) => Boolean = _ => true,
      selectDirections: (direction: Direction, straights: Int) => Seq[Direction]
  ) =
    ...
    while (keepSearching && queue.size > 0)
      ...
      if (loss >= optimalLoss) // We are only looking at worse paths from now
        ...
        val nextDirs = selectDirections(dir, straights)
